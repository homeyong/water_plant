import {
  ...
  const createdSFT = await metaplex.nfts().createSft({
    uri: "https://raw.githubusercontent.com/wwwMalcolm/testfileupload/main/harvestbuddy",
    name: "Harvest Farm",
    symbol: "FARM",
  ...
  async function logTransaction(txHash) {
    ...
    console.log(
      `Solana Explorer: https://explorer.eclipse.com/tx/${txHash}?cluster=testnet`
    );
  }
  ...
  txHash = await pg.program.methods
    .transferIn(new anchor.BN(2 * mintDecimals))
    .accounts({
      tokenAccountOwnerPda: tokenAccountOwnerPda,
      vaultTokenAccount: tokenVault,
      senderTokenAccount: tokenAccount.address,
  ...
  console.log(`Transfer two token into the vault.`);
  await logTransaction(txHash);
